...
const isSubtype = (type1: TypeTag, type2: TypeTag): boolean => true; // TODO placeholder ret
...
const check = (env: TypeEnv, term: AstNode, type: TypeTag): TypeTag =>
    match(term)
        ...
        .otherwise(term => {
            ...
            return isSubtype(termType, type) ? type : new ErrorTypeTag(term, "Type error");
        });
...
function inferBinExpr(env: TypeEnv, binExpr: BinExpr): TypeTag {
    function checkChildren(env: TypeEnv, binExpr: BinExpr, type: TypeTag): TypeTag {
        const leftType = check(env, binExpr.left, type);
        const rightType = check(env, binExpr.right, type);
        const typesCheck = [leftType, rightType].every(typeTag => typeTag.sameTypeAs(type));
        return typesCheck ? type : new ErrorTypeTag(binExpr, `Type error in binary expression. Expected type to be ${type}, but got ${leftType} and ${rightType}`);
    }

    const isBoolExpr = isComparisonOp(binExpr.op) || (['OpAnd', 'OpOr'].includes(binExpr.op.$type));
    let binType: TypeTag;
    if (isBoolExpr) {
        ...
        return checkChildren(env, binExpr, binType);
    } else if (ARITH_OPERATORS.includes(binExpr.op.$type)) {
        binType = new IntegerTTag();
        return checkChildren(env, binExpr, binType);
    } else {
        return new ErrorTypeTag(binExpr, "Type of binary expression cannot be inferred");
    }
}
...
function functionTTagFromFuncDecl(env: TypeEnv, fundecl: FunDecl): TypeTag {
    const paramAndRetTypes = fundecl.types.map(inferTypeAnnot.bind(undefined, env));
    ...
    typeTag = match(term)
        ...
        .with(P.when(isJoin),
                join => {
                    ...
                    if (isSigTTag(inferredLeft)) {
                        return inferType(env, join.right);
                        // the name `isRelation` is currently misleading; 
                        // right now, 'isRelation' basically amounts to 'is a (field, type) pair'
                    } else {
                        return new ErrorTypeTag(join.left, "Expected something that evaluates to a 'CONCEPT' on the left side of the `s`");
                    }
                })
